...
def id3(df0):
    ...
    entropy = lambda s: -reduce(lambda x, y: x + y,
...
def quest(tree):
    ...
    for i in range(len(tree["edges"])):
        e = tree["edges"][i]
        if i != len(tree["edges"]) - 1:
            ans = input(e["name"] + "? y/n ")
            if ans == "y":
                if len(e["edges"]) != 0:
                    quest(e)
                else:
                    print(str(cstr(e["df"].iloc[:, -1])))
                    return str(cstr(e["df"].iloc[:, -1]))
            else:
                continue
        else:
            if len(e["edges"]) != 0:
...
if __name__ == '__main__':
    data = pd.read_excel('растения.xls', engine='xlrd')
    d = data.to_dict()
    p = {}
    for key in d.keys():
