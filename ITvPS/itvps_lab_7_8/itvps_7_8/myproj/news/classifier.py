...
def read_docs_with_class(classes_name):
    ...
    for class_name in classes_name:
        path = '/Applications/MAMP/htdocs/5 семестр/ВЕБ_9/лаба_9/news_yandex/' + class_name + '_all.txt'
        string = ""
        f = open(path)
...
def tf_idf_test(file, pmi):
    idf = read_idf()
    path = '/Applications/MAMP/htdocs/5 семестр/ВЕБ_9/лаба_9/test/' + file + '.txt'
    string = ""
    f = open(path)
    for line in f.readlines():
        string += line
...
if __name__ == '__main__':
    ...
    # write_tf_idf(tf_idf) #запись в файл tf_idf
    ...
    text = texts[0]
    print(text)
    tf_idf_test = tf_idf_test(text, pmi)
    print("Спорт | Технологии")
    print("Мера сходства", cosine(tf_idf_test, tf_idf))
    print()
